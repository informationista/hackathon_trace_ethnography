...
make_trace_plots <- function(df, start_date, end_date) {
  ...
  p <- ggplot(df, aes(day, time, color = user))  + scale_x_datetime(breaks = date_breaks("2 days"), labels=date_format("%b %d")) + scale_y_datetime(limits = lims, breaks = date_breaks("3 hours"), labels = date_format("%H:%M")) + scale_color_brewer(palette = "Paired") + theme(axis.text.x=element_text(angle=90, margin = margin(t = 5, r = 0, b = 0, l = 0))) + annotate("rect", ymin = as.POSIXct(paste(Sys.Date(), "04:00")), ymax = as.POSIXct(paste(Sys.Date(), "14:00")), xmin = as.POSIXct(paste(start_date, "09:00:00")), xmax = as.POSIXct(paste(end_date, "04:00:00")), fill = "grey", alpha = 0.6) +  geom_point(size = 1) + facet_wrap(~data_source, nrow = 3) 
  return(p)
